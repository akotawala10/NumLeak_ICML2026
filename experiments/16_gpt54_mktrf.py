"""Probe GPT-5.4 on Mkt-RF + S&P 500 + NASDAQ + blind — cross-provider test.

Addresses reviewer concern #1 (single-provider scope). Runs the same
Variant-A numeric-answer template on GPT-5.4 across four probes that
already have Sonnet/Haiku baselines:

- Mkt-RF  (Ken French, Variant A)
- S&P 500 price-return  (yfinance ^GSPC)
- NASDAQ Composite      (yfinance ^IXIC)
- Blind U.S. market excess (no Fama-French cue, Ken French Mkt-RF truth)

40 months per probe × 1 model = 160 queries. Hard budget cap aborts
the run if cost crosses --budget-usd.

Output: results/gpt54_baselines.jsonl + console summary.
"""
...
OUT = REPO / "experiments" / "results" / "gpt54_baselines.jsonl"
DATA = REPO / "data" / "ff"
MODEL = "gpt-5.4"
...
def sample_months(probe: str, n: int, seed: int) -> list[str]:
    # mktrf/blind: Ken French Mkt-RF 1963-07 to 2026-02
    # snp500: yfinance ^GSPC from ~1985
    # nasdaq: yfinance ^IXIC from ~1971
    if probe in ("mktrf", "blind"):
