"""Second non-financial cross-domain replication: U.S. monthly
year-over-year CPI inflation rate (FRED CPIAUCSL, computed as
12-month percent change).

Reviewer concern: domain-portability of the audit framework rests
on UNRATE alone (App. N). A second non-financial series in a
different macro category (price level vs labor) hardens the
domain-generalization claim.

Truth: FRED CPIAUCSL (Consumer Price Index for All Urban Consumers,
seasonally adjusted), with YoY % change computed.

Output: results/cpi_baseline.jsonl + console summary.
"""
...
OUT = REPO / "experiments" / "results" / "cpi_baseline.jsonl"
TRUTH_CSV = REPO / "data" / "macro" / "CPIAUCSL.csv"


def cpi_prompt(month: str) -> str:
    return (
        f"What was the U.S. year-over-year CPI inflation rate "
        f"(BLS Consumer Price Index for All Urban Consumers, "
        f"seasonally adjusted, FRED series CPIAUCSL, 12-month "
        f"percent change) in {format_month_human(month)}? "
        f"Answer with a single signed decimal percent "
        f"(e.g., 8.5 for 8.5\% or -0.4 for -0.4\%) and nothing else."
    )
...
def main() -> None:
    ...
    print(f"CPI YoY probe: {len(months)} months × {len(args.models)} "
          f"models = {len(months)*len(args.models)} queries")
    ...
    with OUT.open("a") as f:
        for model in args.models:
            ...
            ep = eps[model]
            for m in months:
                tt = float(truth.get(pd.Period(m, freq="M"),
                                     float("nan")))
