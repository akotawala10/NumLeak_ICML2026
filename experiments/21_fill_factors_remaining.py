"""Probe GPT-5.4-mini, GPT-5.4-nano, DeepSeek-V3.2, Llama-3.3-70B,
Llama-3.1-8B on SMB / HML / RMW / CMA / Mom (Variant A, n=40 per cell)
to close the 6-factor coverage gap.

Sonnet/Haiku are covered via the main sweep; Opus and GPT-5.4 are
covered by experiments/18_fill_factors.py. This script fills the
remaining 5 models × 5 factors = 25 cells.

Output:
- more_factors.jsonl  (mini, nano, DeepSeek)
- llama_factors.jsonl (Llama-70B, Llama-8B)
"""
...
MODELS = [
    "gpt-5.4-mini", "gpt-5.4-nano", "deepseek-v3.2-azure",
    "llama-3.3-70b-groq", "llama-3.1-8b-groq",
]
...
def out_path_for(model: str) -> Path:
    if "groq" in model:
        return RES / "llama_factors.jsonl"
    return RES / "more_factors.jsonl"


def rpm_for(model: str) -> float:
    return 28.0 if "groq" in model else 60.0
...
def main() -> None:
    ...
    for model in args.models:
        if aborted: break
        ep = eps[model]
        out = out_path_for(model)
        out.parent.mkdir(parents=True, exist_ok=True)
        rpm = rpm_for(model)
        min_interval = 60.0 / max(rpm, 1.0)
    ...
    for (m, f), grp in df.groupby(["model_name", "factor"]):
        ...
        print(f"  {m:25s} × {f:6s}: n={n} parsed={np_} parse={pr:.2f} "
