"""Probe Llama 3.3 70B + Llama 3.1 8B (via Groq free tier) on
Mkt-RF + S&P 500 + NASDAQ + blind probes.

Parallel to experiments/16_gpt54_mktrf.py and 17_opus_baselines.py.
Adds open-weight model coverage (cross-vendor capability tier curve)
and a second non-Anthropic provider — strengthens the
"not Anthropic-specific" claim and the capability-scaled-memorization
finding.

Output: results/llama_baselines.jsonl + console summary.
"""
...
OUT = REPO / "experiments" / "results" / "llama_baselines.jsonl"
DATA = REPO / "data" / "ff"
...
def main() -> None:
    ...
    ap.add_argument("--budget-usd", type=float, default=1.00,
                    help="Hard cap; Groq is free so this only catches surprises.")
    ap.add_argument("--rpm", type=float, default=28.0,
                    help="Soft requests-per-minute cap (Groq free tier ~30 RPM).")
    args = ap.parse_args()
